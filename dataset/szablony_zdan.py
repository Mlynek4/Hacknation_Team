...
# --- GRUPA GWARANTUJĄCA POKRYCIE (Używa każdego placeholdera przynajmniej raz) ---
# Używamy prostych zdań, aby upewnić się, że każdy placeholder jest unikalnie użyty.
coverage_templates = [
...
# ----- TEMPLATE GROUPS (Złożone zdania z wysoką koncentracją czasowników) -----
...
# Wszystkie grupy do losowania (z wyłączeniem coverage_templates)
ALL_GROUPS = [
    administrative_openings,
    administrative_actions,
    administrative_closings,
...
def make_template():
    ...
    # 40% - Otwarcie + Akcja + Zamknięcie (losowo)
    if r < 0.40:
        parts.append(random.choice(administrative_openings))
        parts.append(random.choice(administrative_actions))
        if random.random() < 0.75:
            parts.append(random.choice(administrative_closings))
            
    # 40% - 1-2 zdania z losowej grupy tematycznej
    elif r < 0.80:
        n = random.choice([1, 2])
        group = random.choice(ALL_GROUPS[3:])
        for _ in range(n):
            parts.append(random.choice(group))
            
    # 20% - Otwarcie + Losowe zdanie z grupy tematycznej
    else:
        parts.append(random.choice(administrative_openings))
        group = random.choice(ALL_GROUPS[3:])
        parts.append(random.choice(group))
...
# 1. Gwarancja użycia każdego placeholdera
print("Dodawanie szablonów gwarantujących użycie wszystkich placeholderów...")
templates.extend(coverage_templates)
print(f"Liczba szablonów: {len(templates)}")

# 2. Generowanie reszty szablonów losowo
remaining_to_generate = TARGET - len(templates)
if remaining_to_generate > 0:
    for _ in tqdm(range(remaining_to_generate), desc="Generowanie losowych szablonów"):
...
print("---")
print(f"Wygenerowano i zapisano: {len(templates)} szablonów (Docelowo: {TARGET}).")

# --- WERYFIKACJA ---
used_placeholders = set()
for template in templates:
    ph_in_template = re.findall(r"\[([^\]]+)\]", template)
    used_placeholders.update(ph_in_template)

all_placeholders = set(ph.lower().strip("[]") for ph in PH.values())
